...
لیست فعلی: **v2rayNG** (اندروید), **V2rayN** (ویندوز), **V2Box** (اندروید+آیفون), **Happ** (اندروید+آیفون), **Streisand** (آیفون), **Hiddify** (اندروید+آیفون). دکمه‌ی هر اپ فقط روی همون پلتفرم(‌های) مشخص‌شده فعاله؛ روی بقیه‌ی دستگاه‌ها به‌صورت خاموش/غیرقابل‌کلیک نشون داده می‌شه.

دو نوع دکمه داریم:
- **دکمه‌ی خودکار (deeplink):** با کلیک، مستقیم اپ باز می‌شه و لینک ساب بهش اضافه می‌شه (v2rayNG, V2Box, Happ, Hiddify). اگه اون اپ روی گوشی نصب نباشه، بعد از حدود ۲ ثانیه پیام «این برنامه نصب نیست» نشون داده می‌شه.
- **دکمه‌ی کپی (clipboard):** برای V2rayN و Streisand — چون این دو اپ لینک اختصاصی قابل‌اعتماد برای افزودن خودکار ندارن (تست کردیم؛ لینک قدیمی Streisand ارور Invalid config می‌داد)، این دو فقط لینک ساب رو کپی می‌کنن و یک راهنمای کوتاه نشون می‌دن که داخل اپ از «Import from Clipboard» استفاده کنید.

این اسکیم‌ها رو خود سازنده‌ی هر اپ تعریف می‌کنه و گاهی با آپدیت اپ عوض می‌شن. اگه یک دکمه کار نکرد، فقط باید همون بخش مربوط به همون اپ رو توی `CONFIG.apps` (بالای فایل) آپدیت کنید — بقیه‌ی صفحه بهش وابسته نیست.
