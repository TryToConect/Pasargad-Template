...
```js
const CONFIG = {
  brand: { name: 'اسم برند شما', logoEmoji: '🛡️', logoUrl: '', credit: 'نام‌تون', creditYear: '2026', creditUrl: 'https://...' },
...
تعمداً فقط سه حالت نشون داده می‌شه — نه بیشتر:

- **فعال:** حالت پیش‌فرض.
- **در انتظار فعال‌سازی:** کاربری که هنوز هیچ حجمی مصرف نکرده و انقضایی هم براش ثبت نشده (یعنی شمارش معکوسش با اولین اتصال شروع می‌شه، نه از الان) — بخش انقضا هم براش می‌نویسه «پس از اولین اتصال محاسبه می‌شود».
- **غیرفعال:** وقتی پنل مستقیم این وضعیت رو تزریق کنه (کاربری که دستی توی پنل Disable شده). چون این یک اقدام دستی ادمینه (نه چیزی که از عدد مصرف قابل حدس‌زدنه)، اگه این تزریق در دسترس نباشه، این صفحه به‌جای حدس‌زدن، سراغش نمی‌ره.

توجه: این یعنی وقتی حجم یا زمان یک کاربر تموم بشه ولی ادمین دستی غیرفعالش نکرده باشه، برچسب وضعیت همچنان «فعال» می‌مونه — خودِ حلقه‌های حجم/زمان (که عدد واقعی رو نشون می‌دن) درست کار می‌کنن، فقط برچسب وضعیت جدا این حالت رو نام نمی‌بره. این عمدیه، طبق همون چیزی که خواسته شده بود.
